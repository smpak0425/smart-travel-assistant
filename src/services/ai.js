...
일정 맥락에서 여행지(국가/도시)를 파악하여, 해당 여행지에 맞는 현지 언어와 문화로 3가지 컨텍스트 카드를 만들어 주세요.
...
형식:
{
  ...
  "dining": {
    ...
    "phrase": {
      "local": "현지어로 된 표현의 한글 발음 표기",
      "meaning": "그 표현의 한국어 뜻",
      "original": "현지어 원문",
      "langCode": "현지어 BCP-47 언어 코드 (예: fr, de, it, es, en)"
    }
  }
}
`;
...
- 날짜가 명시되지 않았다면 미래의 한 날짜를 기준으로 요일/일차의 흐름에 맞춰 날짜를 이어가세요.
- 각 일정별 소요 시간을 문맥에 맞게 추정하여 startTimeIso와 endTimeIso를 정확히 구성하세요.
